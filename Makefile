...
LIBC_LINK_FLAGS=-Lext/newlib -lc -lg -lm -Lext/gcc -lgcc -lstdc++ --specs=nano.specs

LINKFLAGS := -nostdlib  -Wl,-T,$(shell pwd)/ext/teensy-cores/teensy3/mk20dx256.ld,--build-id=none -lgcc $(LIBC_LINK_FLAGS)

$(HEX): $(MAIN)
	arm-none-eabi-objcopy -O ihex -j .text -j .fini -j .usbdescriptortable  -j .dmabuffers -j .usbbuffers -j .data -j .bss   $< $@
